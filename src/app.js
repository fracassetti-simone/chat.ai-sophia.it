...
app.get('/api/data', async (req, res) => {
    const request = await fetch('https://dashboard.ai-sophia.it/api/startings', {
...
app.post('/api/starting/:id/reinstall', async (req, res) => {
    ...
    await fsp.writeFile(path.join(process.cwd(), 'startings', 'TrunkClient', id + '.js'), (await fsp.readFile(path.join(process.cwd(), 'startings', 'TrunkClient', 'latest.js'), 'utf-8')).replace('${license_key}', licenseKey));
    await startings[id].start();
    res.json({ ok: true });
});
...
const LATEST_FILE = path.join(process.cwd(), 'startings', 'TrunkClient', 'latest.js');
...
const broadcastPendingUpdateAll = (reason = 'latest.js updated') => {
